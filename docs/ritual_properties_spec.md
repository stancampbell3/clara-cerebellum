...
**Status (2026-09-24):** **Parent deadline on the envelope built and live-verified** (Rust + lildaemon, uncommitted at time of writing) — closes S7's known limit. New optional `deadline_ms` on `TephraEnvelope`/`Routing` (`clara-ritual/src/envelope.rs`): the publishing deduction's *remaining* budget in ms, stamped by `CycleController::publish_evaluator_events` on **Offerings only** (min 1 so spent stays distinguishable from absent; never on hohi/tabu/event; absent when the deduction has no deadline). **Relative, not an instant**, so remote hosts with skewed clocks agree; consumers convert on receipt. lildaemon: `goat/models/performance_deadline.py` (`deadline_scope`/`remaining_s`/`min_budget`, a ContextVar entered inside the executor thread); `RitualParticipant` drops a spent Offering unstarted, times the evaluation out at `min(eval_timeout_s, remaining)`, caps a deduce-capable node's inner `deadline_ms` by the remaining budget; the `ritual` evaluator caps the child by `min(deadline_s, remaining)` and refuses to start once it is gone. Documented in `docs/ritual_typed_edges.md` (wire contract). **Live:** a parent with a 10 s deadline (eval_timeout_s 60) gave its child deduction `deadline_ms` 9880 (S7 alone would have allowed ~58 000); a generous parent left the child at its own 40 000. Tests: workspace 596 passed (+7); lildaemon full suite green. Remaining limits: consumer/Kafka lag before receipt is not counted; cancellation stays cooperative (a child blocked in FFI runs to its end); replies are not bounded.

**Status (2026-09-25):** **First real Ritual of Rituals: the frontdesk analysts composed** (`docs/composed_analysts.md`; opt-in `clara` ruleset, deployed). A parent Ritual with a lexical router and five owned child Rituals (the existing analysts, generated from their `.pl` files, system-owned, persistent, versioned) routes each turn; background legs stay in `runtime.py`. Verified live: all five routes, history reaching the child, restart resume, and self-healing rebuild when a child's Dis ritual is gone. Found and fixed in the S6/S7 machinery: `perform()` dropped the conversation context and capped `max_cycles` at 100; evaluator slots keyed by node id alone let a child's `echo` node answer for the parent's `ritual` node (now refused when a slot is held by a different evaluator). Test suites no longer create Rituals in the live Dis (warm-up disabled under test). lildaemon 2118 passed.
