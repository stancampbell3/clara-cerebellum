...
#
# Everything lives inside main(), called only at the very end of this
# file. This script fast-forward-merges clara-cerebellum — the repo this
# very file lives in — as one of its own steps; bash reads a script off
# disk incrementally as it executes top-level statements, so if the
# file's own content shifts mid-run (exactly what that merge does when
# this file itself has upstream changes), execution can silently skip or
# misread whatever comes next. A function body, by contrast, is parsed
# into memory in one shot before any of it runs, so it stays correct
# regardless of what happens to the file on disk afterward. Confirmed
# live 2026-09-08: without this wrapper, a merge that changed this file's
# length caused everything after the merge (a whole new "Front script"
# section, among other things) to be silently skipped on that same run.

set -euo pipefail

main() {
  local SCRIPT_DIR REPO_ROOT DEV_ROOT DOCKER_DIR METADATA
  ...
  local HOST="" DRY_RUN=false ASSUME_YES=false arg
  for arg in "$@"; do
    case "$arg" in
  ...
  local ROLE COMPOSE_FILE ENV_FILE_NAME ENV_EXAMPLE_NAME DIS_BASE_URL_META
  ...
  local ENV_FILE="$DOCKER_DIR/$ENV_FILE_NAME"
  local ENV_EXAMPLE_FILE="$DOCKER_DIR/$ENV_EXAMPLE_NAME"
  ...
  local repo REPO_DIR DIRTY OLD UPSTREAM NEW
