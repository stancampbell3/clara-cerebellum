; Transduced from: nonanimal(X) :- \+ animal(X).
(defrule transduced-nonanimal-on-not_animal-0
    (not_animal ?X)
    =>
...
; Transduced from: nonvertebrata(X) :- animal(X), \+ has(X,backbone).
...
; Transduced from: nonvertebrata(X) :- animal(X), \+ has(X,backbone).
(defrule transduced-nonvertebrata-on-not_has-4
    (not_has ?X backbone)
    =>
    (coire-publish-goal (str-cat "nonvertebrata(" ?X ")")))
...
; Transduced from: mammal(X) :- vertebrata(X), has(X,warm_blooded), \+ has(X,feather).
...
; Transduced from: mammal(X) :- vertebrata(X), has(X,warm_blooded), \+ has(X,feather).
(defrule transduced-mammal-on-not_has-27
    (not_has ?X feather)
    =>
    (coire-publish-goal (str-cat "mammal(" ?X ")")))
